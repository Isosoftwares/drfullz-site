...
import { IconAlertTriangle, IconWallet, IconArrowRight } from "@tabler/icons-react";

function AddDeficitFunds() {
  const axios = useAxiosPrivate();
  const { auth } = useAuth();
  const [searchParams] = useSearchParams(); // <--- 2. Init Hook
  
  // Read the deficit passed from checkout (if any)
  const deficitParam = searchParams.get("deficit"); 
  const deficitAmount = deficitParam ? parseFloat(deficitParam) : 0;
  ...
  const {
    register,
    handleSubmit,
    setValue, // <--- 3. Need setValue to programmatically fill the input
    formState: { errors },
  } = useForm();
  ...
  const formattedArray = currenciesData?.data?.data?.selectedCurrencies.map(
    (currency) => {
      let label;
      switch (currency) {
        case "BTC": label = "Bitcoin (BTC)"; break;
        case "LTC": label = "Litecoin (LTC)"; break;
        case "USDTERC20": label = "USDT (ERC20)"; break;
        case "SOL": label = "Solana (SOL)"; break;
        default: label = currency;
      }
  ...
  // --- 4. NEW LOGIC: Auto-calculate Amount ---
  useEffect(() => {
    // Only run if we have a selected currency and the min amount API has responded
    if (cryptoCurrency && minAmountData?.data?.data?.fiat_equivalent) {
      
      const minLimit = parseFloat(minAmountData.data.data.fiat_equivalent);
      
      // If there is a deficit from checkout
      if (deficitAmount > 0) {
        if (deficitAmount >= minLimit) {
          // Case A: You need $50, Min is $10 -> Set to $50
          setValue("amount", deficitAmount);
          toast.info(`Amount set to required balance: $${deficitAmount}`);
        } else {
          // Case B: You need $5, Min is $10 -> Set to $10
          // Use Math.ceil to ensure clean numbers for crypto limits
          const adjustedAmount = Math.ceil(minLimit); 
          setValue("amount", adjustedAmount);
          toast.warning(
            `You need $${deficitAmount}, but the minimum deposit for this coin is $${adjustedAmount}. Amount adjusted.`
          );
        }
      }
    }
  }, [minAmountData, cryptoCurrency, deficitAmount, setValue]);

  // ... (Rest of your existing Mutation and Select Styles logic remains the same) ...
  ...
  const submit = (data) => {
    ...
    const minAmount = parseFloat(minAmountData?.data?.data?.fiat_equivalent || 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* 5. Optional: User Notification Banner */}
      {deficitAmount > 0 && (
        <div className="bg-blue-900/30 border border-blue-700 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-bold text-sm">Complete your purchase</h3>
            <p className="text-slate-300 text-xs">
              You need <span className="text-white font-bold">${deficitAmount}</span> to finish your order. 
              Select a currency below to calculate the exact deposit.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  {...register("amount", {
                    required: true,
                    // Validation: Must be greater than Min Amount
                    validate: (value) =>
                      !cryptoCurrency ||
                      parseFloat(value) >=
      ...
      {/* Transactions History Table - Same as before... */}
                  ...
                  <tr key={index} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                          item.status?.toLowerCase() === "confirmed" || item.status === "Approved"
                    ...
                    <td className="px-6 py-4 font-mono text-xs max-w-[150px] truncate" title={item.payAddress}>
                      {item.payAddress}
                    </td>
                    <td className="px-6 py-4 uppercase">{item.network}</td>
                    <td className="px-6 py-4 text-right font-medium text-white">${item.priceAmount}</td>
                  </tr>
                ))
              )}
            </tbody>
...
export default AddDeficitFunds;